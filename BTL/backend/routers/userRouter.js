...
userRouter.post('/signin' , expressAsyncHandler(async (req, res) => {
  ...
  res.status(401).send({ message: 'Tên tài khoản hoặc mật khẩu không chính xác!' });
}));
...
userRouter.delete(
  ...
  expressAsyncHandler(async (req, res) => {
    const user = await User.findById(req.params.id);
    if (user) {
      if (user.email === 'ManhCT@123.com.vn') {
        res.status(400).send({ message: 'Không thể xóa tài khoản admin' });
        return;
      }
      const deleteUser = await user.remove();
      res.send({ message: 'Tài khoản người dùng đã xóa!', user: deleteUser });
    } else {
      res.status(404).send({ message: 'Tài khoản không tồn tại!' });
    }
  })
