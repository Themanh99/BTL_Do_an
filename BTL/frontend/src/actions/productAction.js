...
import { CATE_PRO_FAIL, CATE_PRO_REQUEST, CATE_PRO_SUCCESS, PRODUCT_CREATE_FAIL,
...
export const listProductCategories = () => async (dispatch) => {
  dispatch({
    type: CATE_PRO_REQUEST,
  });
  try {
    const { data } = await Axios.get(`/api/products/categories`);
    dispatch({ type: CATE_PRO_SUCCESS, payload: data });
  } catch (error) {
    dispatch({ type: CATE_PRO_FAIL, payload: error.message });
  }
};
...
export const createProduct = () => async (dispatch, getState) => {
  ...
  export const topProduct = () => async (dispatch) => {
    dispatch({ type: TOPSP_REQUEST });
    try {
      const { data } = await Axios.get('/api/products/top-product');
      dispatch({ type: TOPSP_SUCCESS, payload: data });
    } catch (error) {
      const message =
        error.response && error.response.data.message
          ? error.response.data.message
          : error.message;
      dispatch({ type: TOPSP_FAIL, payload: message });
    }
  };
